...
void IngamePathfinder::calculateDistanceToNode(const MonsterNode &node, const vector2d<BUILDING_TYPE> &map, vector2d<int> &distance)
{
	...
	if ((g_game.ingameBuildingSize == 2) && ((nx != (int)(node.x + 0.25f)) || (ny != (int)(node.y + 0.25f))))
	{
		// node is aligned between tiles, start with 2x2
		distance.at(ny+1, nx) = 0;     queue.emplace(ny+1, nx);
		distance.at(ny, nx+1) = 0;     queue.emplace(ny, nx+1);
		distance.at(ny+1, nx+1) = 0;   queue.emplace(ny+1, nx+1);
	}
	...
	while (!queue.empty())
	{
		...
		if ((x < g_game.ingameMapWidth-1) && isPathable(map.at(y, x+1)) && (distance.at(y, x+1) > d))
		{
			distance.at(y, x+1) = d;
			queue.emplace(y, x+1);
		}
		...
		if ((y < g_game.ingameMapHeight-1) && isPathable(map.at(y+1, x)) && (distance.at(y+1, x) > d))
		{
			distance.at(y+1, x) = d;
			queue.emplace(y+1, x);
		}
...
void IngamePathfinder::recalculateNodePaths(IngameMap &map, MonsterNode *pNode)
{
	...
	for (int y = 0; y < g_game.ingameMapHeight; ++y)
	{
		for (int x = 0; x < g_game.ingameMapWidth; ++x)
		{
			...
			// Diagonals
			// Only follow diagonals if further than 8 tiles from source in GCCS, because... reasons.
			if ((g_game.game == GC_LABYRINTH) || (current > 8))
			{
				if (y > 0)
				{
					if (x > 0)
					{
						...
						// Weigh more heavily if heading towards orb, but occasionally shuffle around
						if (N && W && (diff < 1))
