...
IngameEnemyController::IngameEnemyController() : monstersOnTile(g_game.ingameMapHeight, g_game.ingameMapWidth) {}
...
void IngameEnemyController::spawnMonsters(const IngamePathfinder &pathfinder, int num)
{
	...
	for (int i = 0; i < num; ++i)
	{
		...
		monsters.emplace_back(nodes[(797*nodeOffset + 337*i) % nodes.size()], pathfinder.getOrbNode());
	}
	++nodeOffset;
...
void IngameEnemyController::tickMonsters(IngameMap &map, int frames)
{
	...
	for (Monster &m : monsters)
	{
		...
		if      (ix == -1)                      ix = 0;
		else if (ix == g_game.ingameMapWidth)   ix = g_game.ingameMapWidth - 1;
		if      (iy == -1)                      iy = 0;
		else if (iy == g_game.ingameMapHeight)  iy = g_game.ingameMapHeight - 1;
...
void IngameEnemyController::render(struct _fbg *pFbg, const Window &window)const
{
	...
	{
		if ((m.x > 0.0f) && (m.x < g_game.ingameMapWidth) && (m.y > 0.0f) && (m.y < g_game.ingameMapHeight))
		{
			fbg_pixel(pFbg, scale * m.x + window.x, scale * m.y + window.y,
					(m.color >> 4) & 0xFF, (m.color >> 2) & 0xFF, m.color & 0xFF);
		}
	}
}

std::vector<Monster*> IngameEnemyController::getLiveTargetsWithinRangeSq(float y, float x, float rangeSq)
{
	std::vector<Monster*> targets;

	for (Monster &m : monsters)
	{
		if (!m.isKilled && !m.isKillingShotOnTheWay && ((m.y-y)*(m.y-y) + (m.x-x)*(m.x-x) <= rangeSq))
			targets.push_back(&m);
	}
