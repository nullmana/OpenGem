...
STATUS IngameMap::render(struct _fbg *pFbg, const Window &window)const
{
	...
	for (int j = 0; j < g_game.ingameMapHeight; ++j)
	{
		for (int i = 0; i < g_game.ingameMapWidth; ++i)
		{
			...
			fbg_rect(pFbg,
					window.x+i*scale, window.y+j*scale,
					scale, scale,
					(TILE_COLOR[type] >> 4) & 0xFF,
					(TILE_COLOR[type] >> 2) & 0xFF,
...
bool IngameMap::verifyBuilding(BUILDING_TYPE building, int x, int y)
{
	...
	if ((building != BUILDING_TRAP) && overlapsPath && pathfinder.checkBlocking(*this, x, y, bw, bh))
	{
		printf("Blocking!\n");
		return false;
	}
...
STATUS IngameMap::buildWall(int x1, int y1, int x2, int y2)
{
	...
	for (int x = x1; x <= x2; ++x)
	{
		for (int y = y1; y <= y2; ++y)
		{
			if ((tileOccupied.at(y, x) == BUILDING_PATH) || (tileOccupied.at(y, x) == BUILDING_WALL_PATH))
