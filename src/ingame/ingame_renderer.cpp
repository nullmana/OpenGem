...
IngameRenderer::IngameRenderer(IngameCore &core) : windowMap(core.map), windowInventory(core.inventory)
{
...
void IngameRenderer::resize(int width, int height)
{
	...
	windowInventory.resize(offsetX + realWidth + 8, offsetY + (realHeight-invHeight)/2 , invWidth, invHeight);
}
